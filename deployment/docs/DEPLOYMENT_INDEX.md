...
| `deployment/docs/CX5106_WIRE_ID_FIELD_PROCEDURE.md` | **[v1.0.0 — 2026-07-20 S102]** Printable on-boat field procedure to identify CX5106 sender/tach wires **by multimeter, not by colour** on the Monterey 265 SEL. Meter-based I/S/G/L identification, analog-meter settings (Ohms Rx1 + zero-ohms adjust, DC Volts 50V range), per-gauge "make it move" proof, safety rules, and a fill-in results table. Written because Monterey did not follow the standard colour code (no pink/gray, two blues) — colour-based ID failed 3× in S101. Supersedes the colour-based identification in `CX5106_WIRING_CALIBRATION.md` (which needs correction). Relates to BUG-14, BUG-17. Pending on-boat verification, then publish to d3kOS docs (OB-E03). |
