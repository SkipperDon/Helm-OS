...
| `deployment/docs/CX5106_WIRING_CALIBRATION.md` | **[v1.0.0 — 2026-07-10 S95]** CX5106 analog-to-NMEA 2000 converter wiring and calibration guide for Monterey 265 SEL. Covers: 3-wire dash harness colour identification (black=ground, purple=12V ignition, blue=sender signal), parallel wiring diagram, US vs. European sender resistance standards (240–33 Ω vs. 0–190 Ω), two-stage calibration procedure, Signal K path verification, and fine-tune offsets. Research-based; awaiting on-boat verification. Relates to BUG-11. |
| `deployment/docs/CX5106_WIRE_ID_FIELD_PROCEDURE.md` | **[v1.1.0 — 2026-07-20 S103]** Printable on-boat field procedure to identify CX5106 sender/tach wires **by multimeter, not by colour** on the Monterey 265 SEL. Meter-based I/S/G/L identification, digital-meter settings (AstroAI 2000 Counts: Ω→2kΩ for resistance, DC V→20V for voltage), per-gauge "make it move" proof, safety rules, and a fill-in results table. Written S102 because Monterey did not follow the standard colour code — colour-based ID failed 3× in S101. Updated S103 to correct analog-meter instructions (Rx1/50V/zeroing/needle language) for operator's actual AstroAI digital meter. Supersedes the colour-based identification in `CX5106_WIRING_CALIBRATION.md` (which needs correction). Relates to BUG-14, BUG-17. Pending on-boat verification, then publish to d3kOS docs (OB-E03). |
...
| `deployment/docs/V09995_DISCUSSION_PLAN.md` | **[v1.1 — S99 2026-07-15]** v0.9.9.5 Research & Discussion Backlog. 13 research topics (D01–D13): WiBoat WiFi CSI radar (DESIGN COMPLETE), SafeHelm collision avoidance (DESIGN COMPLETE), Sail-O-Vision collaboration, D3K-Lite OpenPlotter addon, signalk-forward-watch v0.2.4, AAO data collection system, Mobile PWA remaining features, pypilot integration (DEFERRED), research paper submission, AAO methodology evolution, 360° Marine Vision NCS2, On-Boat Validation (prerequisite), **D13 VHF Communication Suite** (Audio Replay + STT + Translation + Signal K Alerts — base: github.com/hwater/vhf-clear-audio-replay MIT, 4-phase, port 8088). Includes prioritization table. |
