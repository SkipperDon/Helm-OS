...
| `deployment/pi_config/d3kos-browser.desktop` | **[UPDATED — S55]** `--start-maximized` + trailing URL → `--app=http://localhost/ --start-maximized`. App mode removes address bar and browser tabs entirely — cleaner helm display. Window still maximised. Previous change (S52): `--start-fullscreen` → `--start-maximized`. Pi: `/home/d3kos/.config/autostart/d3kos-browser.desktop` — deployed S55. **Takes effect on next Pi reboot.** |
